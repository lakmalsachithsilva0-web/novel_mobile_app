    ...
    <div className="app-shell">
      ...
      <div className="main-col">
        ...
        <main className="content">
          ...
          {page === "reports" && <ReportsPage books={books} authors={authors} />}
...
function ReportsPage({ books, authors }) {
  const genreData = useMemo(() => {
    ...
    return Object.entries(m).map(([label, value]) => ({ label: label.slice(0, 10), value }));
  }, [books]);
  return (
    <>
      <div className="stats-row">
        <div className="stat-card"><div className="label">Novels</div><p className="value">{books.length}</p></div>
        <div className="stat-card"><div className="label">Authors</div><p className="value">{authors.length}</p></div>
        <div className="stat-card"><div className="label">Genres</div><p className="value">{genreData.length}</p></div>
        <div className="stat-card"><div className="label">Avg rating</div>
          <p className="value">
            {books.length
              ? (books.reduce((s, b) => s + Number(b.rating || 0), 0) / books.length).toFixed(1)
              : "—"}
          </p>
        </div>
      </div>
      <div className="panel-grid">
        <div className="panel">
          <div className="panel-header"><h3>Genre distribution</h3></div>
          <BarChart data={genreData.length ? genreData : [{ label: "—", value: 1 }]} color="#8b5cf6" />
        </div>
        <div className="panel">
          <div className="panel-header"><h3>Status mix</h3></div>
          <BarChart
            data={[
              { label: "Pub", value: books.filter((b) => /publish/i.test(b.status_text || "")).length },
              { label: "Draft", value: books.filter((b) => /draft/i.test(b.status_text || "")).length },
              { label: "Pend", value: books.filter((b) => /pending|review/i.test(b.status_text || "")).length },
            ]}
            color="#14b8a6"
          />
        </div>
